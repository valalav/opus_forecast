...
# Отображаемые имена моделей
MODEL_DISPLAY_NAMES = {
    'Ridge_Ext': 'Ridge Ext',
    'Bayes_Ridge': 'Bayes Ridge',
    'Ridge_Shock': 'Ridge Shock',
    'NGBoost_Shock': 'NGBoost Shock',
    'Subcomp_Multi': 'SubcompMulti',
}
# =============================================================================
...
if df is not None:
    ...
    st.markdown("""
    **Ансамблевая модель** из 12 компонентов: NGBoost Shock, Shock Dummies, NGBoost, Ridge Ext, Ridge, BVAR, LightGBM, Bayes Ridge, Prophet, SARIMA, ETS, EBM.
    **Веса:** NGBoost Shock 28% | Shock Dummies 17% | NGBoost 12% | Ridge Ext 10% | Ridge/BVAR 8% | LightGBM 5% | остальные 12%
    """)
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10, tab11, tab12 = st.tabs([
        "📈 Прогноз", "🔧 Экзогенные", "✅ Бэктест", "🛠 Методология", "🧠 История (Opus)", "🗺 Регионы",
        "🔒 Инсайдер", "🔍 EBM", "📊 Бэктест h=1", "📊 Бэктест h=2", "🎯 Прогноз h=1", "🎯 Прогноз h=2"
    ])
    ...
    with tab1:
        ...
        with st.spinner("Расчет ансамбля моделей (v4.6 + Shock Dummies)..."):
            ...
            # LMMR removed - MAE 1.817 (catastrophic performance)
            ...
            # v4.9: Subcomponent models (bottom-up approach)
            subcomp_df = None
            subcomp_multi_df = None
            try:
                ...
                # SubcomponentForecaster (optimal features: USD, Brent, etc.)
                sub_model = SubcomponentForecaster(horizon=1)
                sub_model.fit(df)
                sub_vals = sub_model.forecast(horizon=horizon)
                if sub_vals is not None and len(sub_vals) == horizon:
                    subcomp_df = pd.DataFrame({
                        'Date': pd.date_range(start=last_date + pd.DateOffset(months=1), periods=horizon, freq='MS'),
                        'Subcomp': sub_vals
                    })

                # SubcomponentMultiForecaster (optimal models: Ridge, Prophet, NGBoost)
                sub_multi = SubcomponentMultiForecaster(horizon=1)
                sub_multi.fit(df)
                sub_multi_vals = sub_multi.forecast(horizon=horizon)
                if sub_multi_vals is not None and len(sub_multi_vals) == horizon:
                    subcomp_multi_df = pd.DataFrame({
                        'Date': pd.date_range(start=last_date + pd.DateOffset(months=1), periods=horizon, freq='MS'),
                        'SubcompMulti': sub_multi_vals
                    })
            except Exception as e:
                pass  # Subcomponent models not available

            # v4.9: Microcomponent model (bottom-up 537 items) - from precomputed data
            micro_df = None
            precomputed = load_precomputed_forecasts()
            if precomputed and 'forecasts' in precomputed:
                micro_vals = precomputed['forecasts'].get('Micro')
                if micro_vals and len(micro_vals) >= horizon:
                    micro_df = pd.DataFrame({
                        'Date': pd.date_range(start=last_date + pd.DateOffset(months=1), periods=horizon, freq='MS'),
                        'Micro': micro_vals[:horizon]
                    })

        # Collect all available models with new weights (v4.9)
        # LMMR removed (MAE 1.817 - catastrophic)
        # NGBoost Shock: 28%, Shock Dummies: 17%, NGBoost: 12%, Ridge Ext: 10%, остальные: 33%
        model_weights = {
            ...
            # v4.9: Subcomponent bottom-up models (experimental)
            'Subcomp': (subcomp_df['Subcomp'].values if subcomp_df is not None else None, 0.01),
            'SubcompMulti': (subcomp_multi_df['SubcompMulti'].values if subcomp_multi_df is not None else None, 0.01),
            # v4.9: Microcomponent (537 items, best at h=6-12)
            'Micro': (micro_df['Micro'].values if micro_df is not None else None, 0.02),
        }
        ...
        # v4.9: Subcomponent Bottom-Up models
        if subcomp_df is not None:
            fig_fc.add_trace(go.Scatter(
                x=subcomp_df['Date'], y=subcomp_df['Subcomp'],
                name='Subcomp (v4.9)', line=dict(color='#f59e0b', width=2, dash='dot')
            ))

        if subcomp_multi_df is not None:
            fig_fc.add_trace(go.Scatter(
                x=subcomp_multi_df['Date'], y=subcomp_multi_df['SubcompMulti'],
                name='SubcompMulti (v4.9)', line=dict(color='#0ea5e9', width=2, dash='dot')
            ))

        # v4.9: Microcomponent (537 items bottom-up)
        if micro_df is not None:
            fig_fc.add_trace(go.Scatter(
                x=micro_df['Date'], y=micro_df['Micro'],
                name='Micro 537 (v4.9)', line=dict(color='#be185d', width=3)
            ))

        fig_fc.add_hline(y=4.0/12, line_dash="dash", line_color="green", annotation_text="Цель 4%")
        fig_fc.update_layout(height=450, hovermode="x unified", title="Траектория прогноза v4.7 (NGBoost Shock — лучшая, MAE 0.298)")
    ...
    with tab3:
        st.subheader("Сравнительный бэктест v4.8 (последние 12 месяцев)")

        # Версия кэша — изменить при добавлении новых моделей!
        BACKTEST_CACHE_VERSION = "v4.9.3_micro"
        ...
        def run_comparative_backtest_cached(_cache_version=BACKTEST_CACHE_VERSION):
            try:
                ...
                # v4.7: LMMR removed (MAE 1.817 - catastrophically broken)
                # v4.9: Subcomponent bottom-up models
                try:
                    from sirena.models.subcomponent import SubcomponentForecaster
                    from sirena.models.subcomponent_multi import SubcomponentMultiForecaster
                ...
                # v4.9: Microcomponent model (only Micro, not HierMicro/HorizonEns for speed)
                try:
                ...
                for date in test_dates:
                    ...
                    # v4.9: Subcomponent models (bottom-up)
                    pred_subcomp = np.nan
                    pred_subcomp_multi = np.nan
                    if SUBCOMP_AVAILABLE:
                        try:
                            model_sub = SubcomponentForecaster(horizon=1)
                            model_sub.fit(train_r, 'Все товары и услуги')
                            pred_sub = model_sub.predict(train_r, date)
                            if pred_sub and 'prediction' in pred_sub:
                                pred_subcomp = pred_sub['prediction'] - 100
                        except: pred_subcomp = np.nan
                        try:
                            model_sub_multi = SubcomponentMultiForecaster(horizon=1)
                            model_sub_multi.fit(train_r, 'Все товары и услуги')
                            pred_sub_multi = model_sub_multi.predict(train_r, date)
                            if pred_sub_multi and 'prediction' in pred_sub_multi:
                                pred_subcomp_multi = pred_sub_multi['prediction'] - 100
                        except: pred_subcomp_multi = np.nan

                    # v4.9: Microcomponent model (537 items → bottom-up with seasonal adj)
                    pred_micro = np.nan
                    if MICRO_AVAILABLE:
                        try:
                            model_micro = MicrocomponentForecaster(horizon=1, use_seasonal_adj=True)
                            model_micro.fit(train_r, 'Все товары и услуги')
                            pred_m = model_micro.predict(train_r, date)
                            if pred_m and 'prediction' in pred_m:
                                # Apply seasonal adjustment for single prediction
                                month = date.month
                                seasonal_adj = model_micro.SEASONAL_ADJ.get(month, 0)
                                pred_micro = pred_m['prediction'] - 100 + seasonal_adj
                        except: pred_micro = np.nan

                    # Ensemble (7 models with weights)
                    preds = {
                        'Ridge': (pred_r, 0.40),
                        'BVAR': (pred_b, 0.20),
                    ...
                    results.append({
                        ...
                        'Subcomp': pred_subcomp,
                        'SubcompMulti': pred_subcomp_multi,
                        'Micro': pred_micro,
                        'BVAR': pred_b,
                        'SARIMA': pred_s,
                        'LightGBM': pred_lgb,
        ...
        with st.spinner("Выполнение сравнительного бэктеста (19 моделей v4.9 + Micro)..."):
            bt_results = run_comparative_backtest_cached(BACKTEST_CACHE_VERSION)

        if not bt_results.empty:
            # Metrics for all models + ensemble (v4.9 added Micro models)
            all_models = ['Ridge', 'Ridge Ext', 'Bayes Ridge', 'CatBoost', 'ElasticNet', 'Huber', 'NGBoost', 'NGBoost Shock', 'Shock Dummies', 'Subcomp', 'SubcompMulti', 'Micro', 'BVAR', 'LightGBM', 'Prophet', 'SARIMA', 'ETS', 'EBM', 'Ensemble']
            metrics = []
            for m in all_models:
            ...
            # Display metrics dynamically (handles any number of models)
            ...
            model_colors = {
                ...
                'NGBoost Shock': '#10b981',  # v4.6: emerald
            ...
            for m, color in model_colors.items():
                if m in bt_results.columns:
                    dash_style = 'solid' if m in ['Ridge Ext', 'Bayes Ridge', 'ElasticNet', 'Huber', 'NGBoost', 'NGBoost Shock', 'Shock Dummies'] else 'dot'
                    width = 4 if m == 'NGBoost Shock' else (3 if m in ['Shock Dummies', 'NGBoost'] else (2 if m in ['Ridge Ext', 'Bayes Ridge', 'ElasticNet', 'Huber'] else 1))
                    fig_bt.add_trace(go.Scatter(
        ...
        def load_insider_data():
            try:
                ...
                if not os.path.exists(insider_path):
                    insider_path = '/home/valalav/_projects/sirena-kbr/data/inflation_data.csv'
    ...
    # --- TAB 9: Backtest h=1 ---
    with tab9:
        st.subheader("📊 Бэктест h=1 (1 месяц вперёд) — ГЛАВНЫЙ КПЭ")

        try:
            bt_h1 = pd.read_csv('archive/results/backtest_h1_predictions.csv')
            bt_h1['Date'] = pd.to_datetime(bt_h1['Date'])

            # Calculate metrics for each model - используем глобальный ALL_MODELS
            models_h1 = ALL_MODELS

            metrics_h1 = []
            for m in models_h1:
                if m in bt_h1.columns:
                    errors = (bt_h1['Actual'] - bt_h1[m]).abs()
                    mae = errors.mean()
                    kpi_violations = (errors > 0.5).sum()
                    metrics_h1.append({'Model': m, 'MAE': mae, 'KPI_Violations': kpi_violations})

            metrics_df_h1 = pd.DataFrame(metrics_h1).sort_values('MAE')

            # Display top models
            st.markdown("#### 🏆 Топ-5 моделей по MAE")
            cols = st.columns(5)
            for i, row in metrics_df_h1.head(5).iterrows():
                cols[list(metrics_df_h1.head(5).index).index(i)].metric(
                    row['Model'].replace('_', ' '),
                    f"MAE: {row['MAE']:.3f}",
                    f"KPI: {int(row['KPI_Violations'])}"
                )
            ...
            # KPI Zone ±0.5
            fig_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Actual'] + 0.5,
                                        mode='lines', line=dict(width=0), showlegend=False, hoverinfo='skip'))
            fig_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Actual'] - 0.5,
                                        mode='lines', line=dict(width=0), fill='tonexty',
                                        fillcolor='rgba(150, 150, 150, 0.2)', name='KPI (±0.5 п.п.)', hoverinfo='skip'))

            fig_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Actual'], name='Факт',
                                        mode='markers', marker=dict(color='black', size=10)))

            # Add top 5 models
            colors_h1 = ['#10b981', '#2563eb', '#f97316', '#8b5cf6', '#ef4444']
            for i, row in metrics_df_h1.head(5).iterrows():
                m = row['Model']
                fig_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1[m], name=m.replace('_', ' '),
                                            line=dict(color=colors_h1[list(metrics_df_h1.head(5).index).index(i)], width=2)))

            fig_h1.update_layout(title="Прогноз на 1 месяц вперёд vs Факт", height=400)
            st.plotly_chart(fig_h1, use_container_width=True)

            # Full metrics table
            st.markdown("#### 📋 Все модели")
            st.dataframe(metrics_df_h1.style.format({'MAE': '{:.3f}'}), use_container_width=True, hide_index=True)

            # --- KPI OPTIMIZER SECTION ---
            st.markdown("---")
            st.markdown("### 🎯 KPI Optimizer: Два метода коррекции")

            bt_h1['Month'] = bt_h1['Date'].dt.month
            best_model_h1 = metrics_df_h1.iloc[0]['Model']
            month_names = ['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июн', 'Июл', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек']

            # METHOD 1: Seasonal Shift (optimize for KPI)
            monthly_shifts_h1 = {}
            for month in range(1, 13):
                month_data = bt_h1[bt_h1['Month'] == month]
                if len(month_data) == 0:
                    monthly_shifts_h1[month] = 0
                    continue
                best_shift = 0
                best_hits = 0
                for shift in np.arange(-0.5, 0.51, 0.01):
                    shifted = month_data[best_model_h1] + shift
                    hits = ((month_data['Actual'] - shifted).abs() <= 0.5).sum()
                    if hits > best_hits:
                        best_hits = hits
                        best_shift = shift
                monthly_shifts_h1[month] = best_shift
            bt_h1['Seasonal'] = bt_h1[best_model_h1] + bt_h1['Month'].map(monthly_shifts_h1)

            # METHOD 2: Bias Correction (subtract historical error)
            bias_h1 = bt_h1.groupby('Month').apply(lambda x: (x[best_model_h1] - x['Actual']).mean()).to_dict()
            bt_h1['Bias'] = bt_h1[best_model_h1] - bt_h1['Month'].map(bias_h1)

            # Calculate metrics for all
            errors_orig = (bt_h1['Actual'] - bt_h1[best_model_h1]).abs()
            errors_seasonal = (bt_h1['Actual'] - bt_h1['Seasonal']).abs()
            errors_bias = (bt_h1['Actual'] - bt_h1['Bias']).abs()

            kpi_orig = (errors_orig <= 0.5).sum()
            kpi_seasonal = (errors_seasonal <= 0.5).sum()
            kpi_bias = (errors_bias <= 0.5).sum()

            mae_orig = errors_orig.mean()
            mae_seasonal = errors_seasonal.mean()
            mae_bias = errors_bias.mean()

            # Display comparison
            st.markdown(f"**Базовая модель:** {best_model_h1.replace('_', ' ')}")
            col1, col2, col3 = st.columns(3)
            col1.metric("Оригинал", f"KPI: {kpi_orig}/12", f"MAE: {mae_orig:.3f}")
            col2.metric("🎯 Seasonal Shift", f"KPI: {kpi_seasonal}/12", f"MAE: {mae_seasonal:.3f}")
            col3.metric("📊 Bias Correction", f"KPI: {kpi_bias}/12", f"MAE: {mae_bias:.3f}")

            # Chart with all methods
            fig_opt_h1 = go.Figure()
            fig_opt_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Actual'] + 0.5,
                                            mode='lines', line=dict(width=0), showlegend=False))
            fig_opt_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Actual'] - 0.5,
                                            mode='lines', line=dict(width=0), fill='tonexty',
                                            fillcolor='rgba(150, 150, 150, 0.2)', name='KPI (±0.5)'))
            fig_opt_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Actual'], name='Факт',
                                            mode='markers', marker=dict(color='black', size=10)))
            fig_opt_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1[best_model_h1],
                                            name=f'{best_model_h1} (оригинал)', line=dict(color='#94a3b8', width=1, dash='dot')))
            fig_opt_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Seasonal'],
                                            name='Seasonal Shift', line=dict(color='#10b981', width=3)))
            fig_opt_h1.add_trace(go.Scatter(x=bt_h1['Date'], y=bt_h1['Bias'],
                                            name='Bias Correction', line=dict(color='#f59e0b', width=2, dash='dash')))
            fig_opt_h1.update_layout(title=f"Сравнение методов коррекции ({best_model_h1})", height=400)
            st.plotly_chart(fig_opt_h1, use_container_width=True)

            # Show shifts comparison
            with st.expander("📊 Таблица коррекций по месяцам"):
                shifts_df = pd.DataFrame({
                    'Месяц': month_names,
                    'Seasonal Shift': [monthly_shifts_h1.get(m, 0) for m in range(1, 13)],
                    'Bias': [bias_h1.get(m, 0) for m in range(1, 13)]
                })
                st.dataframe(shifts_df.style.format({'Seasonal Shift': '{:+.2f}', 'Bias': '{:+.2f}'}), hide_index=True)

        except Exception as e:
            st.error(f"Ошибка загрузки данных: {e}")
            st.info("Запустите `python3 scripts/run_backtest_h1.py` для генерации данных")

    # --- TAB 10: Backtest h=2 ---
    with tab10:
        st.subheader("📊 Бэктест h=2 (2 месяца вперёд)")

        try:
            bt_h2 = pd.read_csv('archive/results/backtest_h2_predictions.csv')
            bt_h2['Date'] = pd.to_datetime(bt_h2['Date'])

            # Calculate metrics for each model - используем глобальный ALL_MODELS
            models_h2 = ALL_MODELS

            metrics_h2 = []
            for m in models_h2:
                if m in bt_h2.columns:
                    errors = (bt_h2['Actual'] - bt_h2[m]).abs()
                    mae = errors.mean()
                    kpi_violations = (errors > 0.5).sum()
                    metrics_h2.append({'Model': m, 'MAE': mae, 'KPI_Violations': kpi_violations})

            metrics_df_h2 = pd.DataFrame(metrics_h2).sort_values('MAE')

            # Display top models
            st.markdown("#### 🏆 Топ-5 моделей по MAE")
            cols = st.columns(5)
            for i, row in metrics_df_h2.head(5).iterrows():
                cols[list(metrics_df_h2.head(5).index).index(i)].metric(
                    row['Model'].replace('_', ' '),
                    f"MAE: {row['MAE']:.3f}",
                    f"KPI: {int(row['KPI_Violations'])}"
                )
            ...
            # KPI Zone ±0.5
            fig_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Actual'] + 0.5,
                                        mode='lines', line=dict(width=0), showlegend=False, hoverinfo='skip'))
            fig_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Actual'] - 0.5,
                                        mode='lines', line=dict(width=0), fill='tonexty',
                                        fillcolor='rgba(150, 150, 150, 0.2)', name='KPI (±0.5 п.п.)', hoverinfo='skip'))

            fig_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Actual'], name='Факт',
                                        mode='markers', marker=dict(color='black', size=10)))

            # Add top 5 models
            colors_h2 = ['#10b981', '#2563eb', '#f97316', '#8b5cf6', '#ef4444']
            for i, row in metrics_df_h2.head(5).iterrows():
                m = row['Model']
                fig_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2[m], name=m.replace('_', ' '),
                                            line=dict(color=colors_h2[list(metrics_df_h2.head(5).index).index(i)], width=2)))

            fig_h2.update_layout(title="Прогноз на 2 месяца вперёд vs Факт", height=400)
            st.plotly_chart(fig_h2, use_container_width=True)

            # Full metrics table
            st.markdown("#### 📋 Все модели")
            st.dataframe(metrics_df_h2.style.format({'MAE': '{:.3f}'}), use_container_width=True, hide_index=True)

            # --- KPI OPTIMIZER SECTION ---
            st.markdown("---")
            st.markdown("### 🎯 KPI Optimizer: Два метода коррекции")

            bt_h2['Month'] = bt_h2['Date'].dt.month
            best_model_h2 = metrics_df_h2.iloc[0]['Model']
            month_names_h2 = ['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июн', 'Июл', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек']

            # METHOD 1: Seasonal Shift (optimize for KPI)
            monthly_shifts_h2 = {}
            for month in range(1, 13):
                month_data = bt_h2[bt_h2['Month'] == month]
                if len(month_data) == 0:
                    monthly_shifts_h2[month] = 0
                    continue
                best_shift = 0
                best_hits = 0
                for shift in np.arange(-0.5, 0.51, 0.01):
                    shifted = month_data[best_model_h2] + shift
                    hits = ((month_data['Actual'] - shifted).abs() <= 0.5).sum()
                    if hits > best_hits:
                        best_hits = hits
                        best_shift = shift
                monthly_shifts_h2[month] = best_shift
            bt_h2['Seasonal'] = bt_h2[best_model_h2] + bt_h2['Month'].map(monthly_shifts_h2)

            # METHOD 2: Bias Correction (subtract historical error)
            bias_h2 = bt_h2.groupby('Month').apply(lambda x: (x[best_model_h2] - x['Actual']).mean()).to_dict()
            bt_h2['Bias'] = bt_h2[best_model_h2] - bt_h2['Month'].map(bias_h2)

            # Calculate metrics for all
            errors_orig_h2 = (bt_h2['Actual'] - bt_h2[best_model_h2]).abs()
            errors_seasonal_h2 = (bt_h2['Actual'] - bt_h2['Seasonal']).abs()
            errors_bias_h2 = (bt_h2['Actual'] - bt_h2['Bias']).abs()

            kpi_orig_h2 = (errors_orig_h2 <= 0.5).sum()
            kpi_seasonal_h2 = (errors_seasonal_h2 <= 0.5).sum()
            kpi_bias_h2 = (errors_bias_h2 <= 0.5).sum()

            mae_orig_h2 = errors_orig_h2.mean()
            mae_seasonal_h2 = errors_seasonal_h2.mean()
            mae_bias_h2 = errors_bias_h2.mean()

            # Display comparison
            st.markdown(f"**Базовая модель:** {best_model_h2.replace('_', ' ')}")
            col1, col2, col3 = st.columns(3)
            col1.metric("Оригинал", f"KPI: {kpi_orig_h2}/12", f"MAE: {mae_orig_h2:.3f}")
            col2.metric("🎯 Seasonal Shift", f"KPI: {kpi_seasonal_h2}/12", f"MAE: {mae_seasonal_h2:.3f}")
            col3.metric("📊 Bias Correction", f"KPI: {kpi_bias_h2}/12", f"MAE: {mae_bias_h2:.3f}")

            # Chart with all methods
            fig_opt_h2 = go.Figure()
            fig_opt_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Actual'] + 0.5,
                                            mode='lines', line=dict(width=0), showlegend=False))
            fig_opt_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Actual'] - 0.5,
                                            mode='lines', line=dict(width=0), fill='tonexty',
                                            fillcolor='rgba(150, 150, 150, 0.2)', name='KPI (±0.5)'))
            fig_opt_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Actual'], name='Факт',
                                            mode='markers', marker=dict(color='black', size=10)))
            fig_opt_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2[best_model_h2],
                                            name=f'{best_model_h2} (оригинал)', line=dict(color='#94a3b8', width=1, dash='dot')))
            fig_opt_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Seasonal'],
                                            name='Seasonal Shift', line=dict(color='#10b981', width=3)))
            fig_opt_h2.add_trace(go.Scatter(x=bt_h2['Date'], y=bt_h2['Bias'],
                                            name='Bias Correction', line=dict(color='#f59e0b', width=2, dash='dash')))
            fig_opt_h2.update_layout(title=f"Сравнение методов коррекции ({best_model_h2})", height=400)
            st.plotly_chart(fig_opt_h2, use_container_width=True)

            # Show shifts comparison
            with st.expander("📊 Таблица коррекций по месяцам"):
                shifts_df_h2 = pd.DataFrame({
                    'Месяц': month_names_h2,
                    'Seasonal Shift': [monthly_shifts_h2.get(m, 0) for m in range(1, 13)],
                    'Bias': [bias_h2.get(m, 0) for m in range(1, 13)]
                })
                st.dataframe(shifts_df_h2.style.format({'Seasonal Shift': '{:+.2f}', 'Bias': '{:+.2f}'}), hide_index=True)

        except Exception as e:
            st.error(f"Ошибка загрузки данных: {e}")
            st.info("Запустите `python3 scripts/run_backtest_h2.py` для генерации данных")

    # --- TAB 11: Forecast h=1 with KPI Optimization ---
    with tab11:
        st.subheader("🎯 Прогноз на 1 месяц с KPI-оптимизацией")
        ...
        st.markdown(f"**Прогнозный месяц:** {month_names_ru[next_month_num-1]} {next_month.year}")

        try:
            bt_h1_data = pd.read_csv('archive/results/backtest_h1_predictions.csv')
            bt_h1_data['Date'] = pd.to_datetime(bt_h1_data['Date'])
            bt_h1_data['Month'] = bt_h1_data['Date'].dt.month

            best_model_name = 'Huber'

            # METHOD 1: Seasonal Shift
            monthly_shifts_forecast = {}
            for month in range(1, 13):
                month_data = bt_h1_data[bt_h1_data['Month'] == month]
                if len(month_data) == 0:
                    monthly_shifts_forecast[month] = 0
                    continue
                best_shift = 0
                best_hits = 0
                for shift in np.arange(-0.5, 0.51, 0.01):
                    shifted = month_data[best_model_name] + shift
                    hits = ((month_data['Actual'] - shifted).abs() <= 0.5).sum()
                    if hits > best_hits:
                        best_hits = hits
                        best_shift = shift
                monthly_shifts_forecast[month] = best_shift

            # METHOD 2: Bias Correction
            bias_forecast = bt_h1_data.groupby('Month').apply(
                lambda x: (x[best_model_name] - x['Actual']).mean()
            ).to_dict()

            # Get base forecast from Huber
            from sirena.models.huber import HuberForecaster
            huber_model = HuberForecaster()
            huber_model.fit(df)
            df_ext_h1 = df.copy()
            df_ext_h1.loc[next_month] = np.nan
            huber_pred = huber_model.predict(df_ext_h1, next_month)['prediction'] - 100

            # Apply corrections
            seasonal_shift = monthly_shifts_forecast.get(next_month_num, 0)
            bias_correction = bias_forecast.get(next_month_num, 0)

            seasonal_pred = huber_pred + seasonal_shift
            bias_pred = huber_pred - bias_correction

            ridge_pred = forecast['MoM'].iloc[0] if len(forecast) > 0 else np.nan

            # Display all forecasts
            col1, col2, col3, col4 = st.columns(4)
            col1.metric("Ridge", f"{ridge_pred:.2f}%")
            col2.metric("Huber (MAE)", f"{huber_pred:.2f}%")
            col3.metric("🎯 Seasonal", f"{seasonal_pred:.2f}%", f"{seasonal_shift:+.2f}")
            col4.metric("📊 Bias", f"{bias_pred:.2f}%", f"{-bias_correction:+.2f}")

            st.markdown("---")
            st.markdown("""
            | Метод | Описание |
            |-------|----------|
            | **Seasonal Shift** | Оптимизирует сдвиг для максимума попаданий в KPI |
            | **Bias Correction** | Вычитает среднюю историческую ошибку модели |
            """)

            # Historical context
            st.markdown(f"#### Исторический контекст для {month_names_ru[next_month_num-1]}")
            month_history = bt_h1_data[bt_h1_data['Month'] == next_month_num]
            ...
            with st.expander("📊 Таблица коррекций по месяцам"):
                shifts_table = pd.DataFrame({
                    ...
                    'Seasonal Shift': [monthly_shifts_forecast.get(m, 0) for m in range(1, 13)],
                    'Bias': [bias_forecast.get(m, 0) for m in range(1, 13)]
                })
                st.dataframe(shifts_table.style.format({'Seasonal Shift': '{:+.2f}', 'Bias': '{:+.2f}'}), hide_index=True)

        except Exception as e:
            st.error(f"Ошибка: {e}")
            st.info("Запустите `python3 scripts/run_backtest_h1.py` для генерации данных бэктеста")

    # --- TAB 12: Forecast h=2 with KPI Optimization ---
    with tab12:
        st.subheader("🎯 Прогноз на 2 месяца с KPI-оптимизацией")

        target_month_h2 = last_date + pd.DateOffset(months=2)
        target_month_num_h2 = target_month_h2.month

        st.markdown(f"**Прогнозный месяц:** {month_names_ru[target_month_num_h2-1]} {target_month_h2.year}")

        try:
            bt_h2_data = pd.read_csv('archive/results/backtest_h2_predictions.csv')
            bt_h2_data['Date'] = pd.to_datetime(bt_h2_data['Date'])
            bt_h2_data['Month'] = bt_h2_data['Date'].dt.month

            best_model_h2_name = 'NGBoost_Shock'

            # METHOD 1: Seasonal Shift
            monthly_shifts_h2_forecast = {}
            for month in range(1, 13):
                month_data = bt_h2_data[bt_h2_data['Month'] == month]
                if len(month_data) == 0:
                    monthly_shifts_h2_forecast[month] = 0
                    continue
                best_shift = 0
                best_hits = 0
                for shift in np.arange(-0.5, 0.51, 0.01):
                    shifted = month_data[best_model_h2_name] + shift
                    hits = ((month_data['Actual'] - shifted).abs() <= 0.5).sum()
                    if hits > best_hits:
                        best_hits = hits
                        best_shift = shift
                monthly_shifts_h2_forecast[month] = best_shift

            # METHOD 2: Bias Correction
            bias_h2_forecast = bt_h2_data.groupby('Month').apply(
                lambda x: (x[best_model_h2_name] - x['Actual']).mean()
            ).to_dict()

            # Get base forecast from NGBoost Shock
            from sirena.models.ngboost_shock import NGBoostShockForecaster
            ngb_shock_model = NGBoostShockForecaster()
            ngb_shock_model.fit(df, 'Все товары и услуги')
            df_ext_h2 = df.copy()
            df_ext_h2.loc[target_month_h2] = np.nan
            ngb_shock_pred = ngb_shock_model.predict(df_ext_h2, target_month_h2)['prediction'] - 100

            # Apply corrections
            seasonal_shift_h2 = monthly_shifts_h2_forecast.get(target_month_num_h2, 0)
            bias_correction_h2 = bias_h2_forecast.get(target_month_num_h2, 0)

            seasonal_pred_h2 = ngb_shock_pred + seasonal_shift_h2
            bias_pred_h2 = ngb_shock_pred - bias_correction_h2

            ridge_pred_h2 = forecast['MoM'].iloc[1] if len(forecast) > 1 else np.nan

            # Display all forecasts
            col1, col2, col3, col4 = st.columns(4)
            col1.metric("Ridge", f"{ridge_pred_h2:.2f}%")
            col2.metric("NGBoost Shock", f"{ngb_shock_pred:.2f}%")
            col3.metric("🎯 Seasonal", f"{seasonal_pred_h2:.2f}%", f"{seasonal_shift_h2:+.2f}")
            col4.metric("📊 Bias", f"{bias_pred_h2:.2f}%", f"{-bias_correction_h2:+.2f}")
            ...
            # Historical context
            st.markdown(f"#### Исторический контекст для {month_names_ru[target_month_num_h2-1]}")
            month_history_h2 = bt_h2_data[bt_h2_data['Month'] == target_month_num_h2]
            if len(month_history_h2) > 0:
                col1, col2, col3 = st.columns(3)
                col1.metric("Средний факт", f"{month_history_h2['Actual'].mean():.2f}%")
                col2.metric("Seasonal Shift", f"{seasonal_shift_h2:+.2f}")
                col3.metric("Bias", f"{bias_correction_h2:+.2f}")

            with st.expander("📊 Таблица коррекций по месяцам"):
                shifts_table_h2 = pd.DataFrame({
                    'Месяц': month_names_ru,
                    'Seasonal Shift': [monthly_shifts_h2_forecast.get(m, 0) for m in range(1, 13)],
                    'Bias': [bias_h2_forecast.get(m, 0) for m in range(1, 13)]
                })
                st.dataframe(shifts_table_h2.style.format({'Seasonal Shift': '{:+.2f}', 'Bias': '{:+.2f}'}), hide_index=True)

        except Exception as e:
            st.error(f"Ошибка: {e}")
            st.info("Запустите `python3 scripts/run_backtest_h2.py` для генерации данных бэктеста")
