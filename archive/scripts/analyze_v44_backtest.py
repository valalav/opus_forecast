"""
Подробный анализ бэктеста моделей v4.4 с графиками.
"""
...
print("=" * 60)
print("ПОДРОБНЫЙ АНАЛИЗ БЭКТЕСТА v4.4")
print("=" * 60)
...
plt.suptitle('Бэктест 2023-2025: Прогноз vs Факт (v4.4)', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('backtest_v44_forecast_vs_actual.png', dpi=150, bbox_inches='tight')
print("\nСохранено: backtest_v44_forecast_vs_actual.png")
...
plt.suptitle('Бэктест 2023-2025: Ошибки по месяцам (v4.4)', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('backtest_v44_errors.png', dpi=150, bbox_inches='tight')
print("Сохранено: backtest_v44_errors.png")
...
ax4.set_title('Систематическая ошибка')
for bar, val in zip(bars, me_values):
    ax4.text(val + 0.01 if val >= 0 else val - 0.05, bar.get_y() + bar.get_height()/2, f'{val:.3f}', va='center')

plt.suptitle('Сравнение моделей v4.4', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('backtest_v44_comparison.png', dpi=150, bbox_inches='tight')
print("Сохранено: backtest_v44_comparison.png")
...
ax.set_title('Накопленная ошибка по времени (v4.4)')
ax.legend(loc='upper left')
plt.tight_layout()
plt.savefig('backtest_v44_cumulative.png', dpi=150, bbox_inches='tight')
print("Сохранено: backtest_v44_cumulative.png")

# === АНАЛИЗ NGBoost vs Ridge Extended ===
print("\n" + "=" * 60)
print("СРАВНЕНИЕ ТОПОВЫХ МОДЕЛЕЙ")
print("=" * 60)

if 'Ridge Extended v2' in all_results and 'NGBoost' in all_results:
    rex = all_results['Ridge Extended v2'].sort_values('date')
    ngb = all_results['NGBoost'].sort_values('date')

    print("\nПомесячное сравнение:")
    print(f"{'Дата':<12} {'Факт':>8} {'RidgeExt':>8} {'NGBoost':>8} {'Лучше':>10}")
    print("-" * 50)
    ...
    for i in range(len(rex)):
        date = rex.iloc[i]['date']
        actual = rex.iloc[i]['actual']
        pred_rex = rex.iloc[i]['prediction']
        pred_ngb = ngb.iloc[i]['prediction']

        err_rex = abs(actual - pred_rex)
        err_ngb = abs(actual - pred_ngb)

        if err_rex < err_ngb:
            better = "RidgeExt"
            wins_rex += 1
        elif err_ngb < err_rex:
            better = "NGBoost"
            wins_ngb += 1
        else:
            better = "Ничья"

        print(f"{date.strftime('%Y-%m'):<12} {actual:>8.2f} {pred_rex:>8.2f} {pred_ngb:>8.2f} {better:>10}")

    print("-" * 50)
    print(f"Ridge Extended побед: {wins_rex}")
    print(f"NGBoost побед: {wins_ngb}")
...
print("""
1. Ridge Extended v2 остаётся лучшей моделью (MAE=0.3134)

2. NGBoost показал отличный результат (MAE=0.3148, -0.27% vs baseline):
   - Очень близко к Ridge Extended
   - Даёт вероятностные прогнозы
   - CI слишком узкие (покрытие 20% вместо 90%)

3. Ridge v3 с dummy-переменными хуже (MAE=0.3410, +8%):
   - Dummy для 2010/2022 не помогли
   - Лучше исключать выбросные годы полностью

4. Constrained Components умеренный результат (MAE=0.3314):
   - Компонентный прогноз не улучшил качество
   - Накопление ошибок компонентов

5. Quantile Ridge (MAE=0.3389):
   - Покрытие CI = 50% (неплохо для 80% CI)
   - Интервалы информативные

Рекомендация: Использовать Ridge Extended v2 как основную модель,
NGBoost как альтернативу с вероятностями.
""")
...
print("=" * 60)
print("\nГрафики сохранены:")
print("  - backtest_v44_forecast_vs_actual.png")
print("  - backtest_v44_errors.png")
print("  - backtest_v44_comparison.png")
print("  - backtest_v44_cumulative.png")
