"""
Ridge Production Proxy Rolling
==============================

Производная модель поверх RidgeProductionProxy:
- сохраняет production proxy features из infostat;
- заменяет глобальную seasonal norm на rolling seasonality;
- не изменяет исходный `ridge_production_proxy.py`.
"""
...
@ModelRegistry.register("ridge_production_proxy_rolling_24m")
class RidgeProductionProxyRollingForecaster(RidgeProductionProxyForecaster):
    """RidgeProductionProxy с rolling seasonality на последних N месяцах."""

    name = "ridge_production_proxy_rolling_24m"
    OUTLIER_YEARS = [2010]
    ...
    def _compute_seasonal_norm(self, df: pd.DataFrame) -> pd.Series:
        """Вычислить seasonal norm на последних `seasonality_window` месяцах."""
