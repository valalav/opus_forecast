...
@ModelRegistry.register("ridge_extended_production_proxy_rolling_24m")
class RidgeExtendedProductionProxyRollingForecaster(
    RidgeExtendedProductionProxyForecaster
):
    """RidgeExtendedProductionProxy со скользящей сезонностью по последним N месяцам."""

    name = "ridge_extended_production_proxy_rolling_24m"
    OUTLIER_YEARS = [2010]
    BASE_FEATURES = [
        "y_lag1",
        "y_lag2",
    ...
    def backtest(
        ...
        for target_date in test_dates:
            ...
            try:
                model = RidgeExtendedProductionProxyRollingForecaster(
                    alpha=self.alpha,
