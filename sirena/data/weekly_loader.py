...
def compute_basket_signal(
    ...
    if weighting == "basket":
        ...
        # Use inverse volatility weighting: w = 1/std
        # Calculate volatility from historical data
        volatility_stats = calculate_product_volatility(df=df, lookback_years=3)
        ...
        for code, signal in product_signals.items():
            ...
            # Get volatility for this product
            vol_row = volatility_stats[volatility_stats["product_code"] == code]
            if len(vol_row) == 0:
                std = 5.0  # Default volatility
            else:
                std = vol_row["volatility"].values[0]

            # Inverse volatility weight
            weight = 1.0 / max(std, 0.1)  # Avoid division by zero
    ...
    else:  # weighting == 'equal' or any other value
...
if __name__ == "__main__":
    ...
    # Compute current signal (test different weighting methods)
    print("\nCurrent month signal (basket weighting):")
    signal_basket = compute_basket_signal(weighting="basket")
    print(f"  Overall: {signal_basket['signal']:.3f}%")
    print(f"  Food: {signal_basket['food_signal']:.3f}%")
    print(f"  Non-food: {signal_basket['nonfood_signal']:.3f}%")
    print(f"  Coverage: {signal_basket['coverage'] * 100:.1f}%")

    print("\nCurrent month signal (volatility weighting):")
    signal_vol = compute_basket_signal(weighting="volatility")
    print(f"  Overall: {signal_vol['signal']:.3f}%")
    print(f"  Food: {signal_vol['food_signal']:.3f}%")
    print(f"  Non-food: {signal_vol['nonfood_signal']:.3f}%")
    print(f"  Coverage: {signal_vol['coverage'] * 100:.1f}%")
