"""
Component Ridge Forecaster - Uses component-level features like successful Ridge.

Based on analysis of Ridge Forecaster (MAE 0.321).
Uses the same component-level features: food, nonfood, services.
"""
...
class ComponentRidgeForecaster:
    """
    Component Ridge Forecaster using subcomponent features.

    Key components (from inflation_data.csv):
    - Prod: Food products
    - Nonprod: Non-food products
    - Serv: Services

    This matches the successful Ridge Forecaster approach.
    """

    name = "component_ridge"
    ...
    def __init__(self, alpha: float = None, ets_weights: Dict[int, float] = None):
        """Initialize Component Ridge forecaster."""
        self.alpha = alpha
    ...
    def _create_features(self, df: pd.DataFrame, idx: int) -> Dict[str, float]:
        """Create features using component-level data."""
        features = {}

        # Component lags (key to Ridge success!)
        if "Prod" in df.columns and idx >= 0:
            features["food_lag1"] = df.iloc[idx]["Prod"]
        else:
            if idx >= 0:
                features["food_lag1"] = df.iloc[idx][self.target_col]

        if "Nonprod" in df.columns and idx >= 0:
            features["nonfood_lag1"] = df.iloc[idx]["Nonprod"]
        else:
            if idx >= 0:
                features["nonfood_lag1"] = df.iloc[idx][self.target_col]

        if "Serv" in df.columns and idx >= 0:
            features["services_lag1"] = df.iloc[idx]["Serv"]
        else:
            if idx >= 0:
                features["services_lag1"] = df.iloc[idx][self.target_col]
    ...
    def fit(self, df: pd.DataFrame, target_col: str = "Все товары и услуги"):
        """Fit the Component Ridge model."""
        self.target_col = target_col

        # Check if component data exists
        self.has_components = all(c in df.columns for c in ["Prod", "Nonprod", "Serv"])
...
def main():
    """Test Component Ridge forecaster."""
    df = pd.read_csv(
    ...
    # Select columns - include components!
    df = df[
        ["mom", "Prod", "Nonprod", "Serv", "brent", "usd_nom_i", "Ki", "Ruonia"]
    ].copy()
    df = df.rename(columns={"mom": "Все товары и услуги"})
    df = df.dropna(subset=["Все товары и услуги", "usd_nom_i"])

    print("=" * 70)
    print("Component Ridge Forecaster")
    print("=" * 70)
    print(f"\nData: {len(df)} months ({df.index[0].date()} to {df.index[-1].date()})")
    print(
        f"Components available: Prod={'Prod' in df.columns}, Nonprod={'Nonprod' in df.columns}, Serv={'Serv' in df.columns}"
    )

    model = ComponentRidgeForecaster()
