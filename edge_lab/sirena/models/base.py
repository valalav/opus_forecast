...
class BaseForecaster(ABC):
    ...
    def _fit_uncached(
        ...
        """
        Internal uncached implementation of fit().
        ...
        raise NotImplementedError(
            f"{self.__class__.__name__} must implement _fit_uncached()"
        )
    ...
    def fit(
        ...
        """
        Обучение модели на данных (with caching).
    ...
    def _predict_uncached(
        ...
        """
        Internal implementation of predict (uncached).
        ...
        return {"date": target_date, "prediction": fc[0], "model": self.name}

    if CACHE_ENABLED:
        _predict_impl = cached_predict(_predict_uncached)
    else:
        _predict_impl = _predict_uncached
    ...
    def predict(self, df: pd.DataFrame, target_date: pd.Timestamp) -> Dict[str, Any]:
        ...
        Returns:
            dict с прогнозом и метаданными
        """
        return self._predict_impl(df, target_date)
    ...
    def iterative_forecast(
        ...
        for h in range(horizon):
            ...
            try:
                ...
                if sn is not None and (
                    isinstance(sn, dict) or (hasattr(sn, "__len__") and len(sn) > 0)
                ):
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(name='{self.name}', fitted={self._is_fitted})"
        )
...
class ForecastResult:
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({"Date": self.dates, "Forecast": self.values})
