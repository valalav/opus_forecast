"""
Unit tests for RidgeExtendedForecaster
...
class TestRidgeExtendedForecaster:
    """Test suite for RidgeExtendedForecaster."""
    ...
    def test_import_model(self):
        ...
        assert model.name == "ridge_extended"
    ...
    def test_fit_basic(self, sample_data):
        ...
        model = RidgeExtendedForecaster(use_macro=False)
    ...
    def test_fit_with_macro(self, sample_data_with_macro):
        ...
        model = RidgeExtendedForecaster(use_macro=True)
    ...
    def test_fit_custom_alpha(self, sample_data):
        ...
        custom_alpha = 0.5
        model = RidgeExtendedForecaster(alpha=custom_alpha, use_macro=False)
        model.fit(sample_data)
    ...
    def test_predict_basic(self, sample_data):
        ...
        assert result["model"] == "ridge_extended"
    ...
    def test_backtest_basic(self, sample_data):
        ...
        if not results.empty:
            ...
            assert "pred_ets" in results.columns
    ...
    def test_get_feature_importance(self, sample_data):
        ...
        assert "is_new" in importance.columns
    ...
    def test_get_model_info(self, sample_data):
        ...
        assert info["name"] == "ridge_extended"
    ...
    def test_iterative_forecast(self, sample_data):
        """Test iterative forecast method."""
        from sirena.models.ridge_extended import RidgeExtendedForecaster
        ...
        horizon = 6
        forecast = model.iterative_forecast(sample_data, horizon)

        assert len(forecast) == horizon
        assert all(isinstance(v, (int, float)) for v in forecast)

    def test_iterative_forecast_not_fitted_error(self, sample_data):
        """Test iterative_forecast raises error when not fitted."""
        from sirena.models.ridge_extended import RidgeExtendedForecaster

        model = RidgeExtendedForecaster()

        with pytest.raises(ValueError, match="не обучена"):
            model.iterative_forecast(sample_data, 12)
    ...
    def test_custom_alpha_parameter(self):
        ...
        custom_alpha = 0.1
        model = RidgeExtendedForecaster(alpha=custom_alpha)
    ...
    def test_repr(self):
        ...
        assert "RidgeExtendedForecaster" in repr_str
