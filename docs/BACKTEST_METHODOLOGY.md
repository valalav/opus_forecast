...
## Актуальная общая проверка — 09.09.2026

`BacktestRunner.run_common_origins()` использует одинаковые 24 целевых месяца и рабочие fit/forecast для Ridge, Huber, Ridge_ProdProxy и двух вариантов Micro на h=1/2/12. h=12 здесь — множество отсечек, а не одна траектория. Политика Micro выбирается по первым 12 месяцам; MAE, RMSE, N_valid/N_common и попадание в ±0,5 п.п. последних 12 публикуются отдельно. [Результаты и воспроизведение](../archive/results/micro_policy_20260909/REPORT.md).

В интерфейсе соответствующие вкладки используют проверочные общие даты при совпадении hashes входов. Старые архивы ниже относятся к прежним настройкам; их Micro была внешней ARIMA, а не текущей Microcomponent. Пропуски разных моделей нельзя объединять в общий рейтинг без общего пересечения. Попадание в фиксированный допуск не является покрытием прогнозного интервала. Никакие поправки не подбираются по проверочному периоду.

Следующие разделы сохраняют описание архивных запусков; они не заменяют новую общую проверку и не подтверждают сопоставимость всех моделей.
