...
## Актуальный порядок с 09.09.2026

Для новых исследований действует [план P0–P7](CPI_MODEL_DEVELOPMENT_PLAN.md) и `BacktestRunner.run_registered_experiment`, запускаемый `scripts/run_model_development.py`. Зафиксировать протокол, источники и одинаковые целевые даты до расчета. Исследовательская регистрация не дает автоматического допуска в рабочий Ensemble.

Примеры ниже — историческая схема интерфейса. Не копировать из них live-refit в UI, `except: pass`, автоматическое добавление веса Ensemble и единственную траекторию как rolling h=12. Рабочий UI читает единый проверенный кэш; новый кандидат сначала проходит вспомогательный ввод, а замена основного требует проспективной проверки. Архивные рейтинги другой реализации под тем же именем не наследуются.
