...
# --- SETUP ---
st.set_page_config(
    page_title="СИРЕНА-КБР v3.2: Прогноз Инфляции",
    layout="wide",
...
if df is not None:
    ...
    **MAE:** 0.3317 (Проверена и подтверждена как лучшая)
    ...
    # --- TABS ---
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📈 Прогноз", "🔧 Экзогенные", "✅ Бэктест", "🛠 Методология", "🧠 История (Opus)", "🗺 Регионы"])
    ...
    with tab3:
        st.subheader("Сравнительный бэктест (последние 12 месяцев)")
        
        @st.cache_data
        def run_comparative_backtest_cached():
            try:
                ...
                for date in test_dates:
                    ...
                    try:
                        ...
                        model_b = BayesianVAR(train_b, ['CPI', 'Food', 'USD', 'RUONIA'], lags=4)
                        model_b.fit(lambda1=1.0)
                        fc = model_b.forecast(h=1, n_draws=1000)
                    ...
                    # Ensemble
                    if not np.isnan(pred_r) and not np.isnan(pred_b) and not np.isnan(pred_s):
                        pred_e = 0.6*pred_r + 0.3*pred_b + 0.1*pred_s
                    else:
                        pred_e = pred_r # Fallback
        ...
        with st.spinner("Выполнение сравнительного бэктеста (Ridge vs BVAR vs SARIMA)..."):
            bt_results = run_comparative_backtest_cached()
            
        if not bt_results.empty:
            # Metrics
            metrics = []
            for m in ['Ridge', 'BVAR', 'SARIMA', 'Ensemble']:
                mae = (bt_results['Actual'] - bt_results[m]).abs().mean()
                metrics.append((m, mae))
            
            cols = st.columns(4)
            for i, (m, mae) in enumerate(metrics):
                cols[i].metric(f"{m} MAE", f"{mae:.3f}")
                
            # Plot
            fig_bt = go.Figure()
            fig_bt.add_trace(go.Bar(x=bt_results['Date'], y=bt_results['Actual'], name='Факт', marker_color='#cbd5e1'))
            fig_bt.add_trace(go.Scatter(x=bt_results['Date'], y=bt_results['Ridge'], name='Ridge', line=dict(dash='dot')))
            fig_bt.add_trace(go.Scatter(x=bt_results['Date'], y=bt_results['BVAR'], name='BVAR', line=dict(dash='dot', color='orange')))
            fig_bt.add_trace(go.Scatter(x=bt_results['Date'], y=bt_results['Ensemble'], name='Ансамбль', line=dict(color='#8b5cf6', width=3)))
            fig_bt.update_layout(title="Сравнение точности моделей (1 мес горизонт)", height=400)
            st.plotly_chart(fig_bt, use_container_width=True)
            
            st.subheader("Детальные результаты")
            st.dataframe(bt_results.style.format("{:.2f}"))
