...
class HuberRollingLike(Protocol):
...
def build_model(**kwargs: object) -> Any:
    return HuberRollingForecaster(**kwargs)
...
def test_model_import_and_registry() -> None:
    ...
    registry_model = ModelRegistry.get("huber_rolling_24m")
    assert isinstance(registry_model, HuberRollingForecaster)


def test_model_parameters() -> None:
    model = cast(HuberRollingLike, build_model(use_macro=True, seasonality_window=24))
    assert model.seasonality_window == 24
    assert model.use_macro is True
    assert model.name == "huber_rolling_24m"


def test_fit_predict_backtest(sample_data: pd.DataFrame) -> None:
    model = cast(HuberRollingLike, build_model(use_macro=True, seasonality_window=24))
    ...
    backtest_results = model.backtest(sample_data, start_date="2023-01-01")
...
def test_rolling_seasonal_norm_uses_recent_window() -> None:
    ...
    model = cast(HuberRollingLike, build_model(use_macro=False, seasonality_window=24))
