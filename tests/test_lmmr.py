...
class TestLMMRForecaster:
    ...
    def test_fit(self):
        ...
        assert model.seasonal_component is not None
    ...
    def test_insufficient_data_error(self):
        """Тест ошибки при недостатке данных."""
    ...
    def test_missing_target_column_error(self):
        """Тест ошибки при отсутствии целевой колонки."""
        dates = pd.date_range(start='2020-01-01', end='2024-12-01', freq='MS')
        df = pd.DataFrame({
            'usd_nom_i': [70.0] * len(dates),
            'brent': [60.0] * len(dates)
